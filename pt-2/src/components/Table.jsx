...
const Table = ({ isAdmin }) => {
    ...
    const handleAdminCheck = async (userId) => {
        try {
            const updatedUser = await updateUserInServer(userId, { isAdmin: !isAdminList[userId] });
            console.log(updatedUser);
            // Mevcut kullanıcı bilgilerini değiştirmeden sadece isAdmin değerini güncelle
            const updatedUsers = users.map(user => {
                if (user.id === userId) {
                    return { ...user, isAdmin: !isAdminList[userId] };
                }
                return user;
            });

            // Güncellenmiş kullanıcı listesini Redux store'a gönder
            dispatch(setUsers(updatedUsers));

            // isAdminList'i güncelle
            setIsAdminList(prevState => ({
                ...prevState,
    ...
    const fetchUsers = async () => {
        try {
            ...
            // Kullanıcıları getirirken isAdminList'i güncelleyin
            const isAdminListUpdate = {};
            data.forEach(user => {
                isAdminListUpdate[user.id] = user.isAdmin;
            });
            setIsAdminList(isAdminListUpdate);
                ...
                <table id="personelTable">
                    <thead>
                        <tr>
                            ...
                            <th>Mail</th>
                            <th>TELEFON</th>
                            <th className='tdCenter'>IsAdmin</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        {users.map(user => (
                            <tr key={user.id}>
                                ...
                                <td>{user.mail}</td>
                                <td>{user.tel}</td>
                                <td className='tdCenter'><input type="checkbox" checked={isAdminList[user.id]} onChange={() => handleAdminCheck(user.id)} /></td>
                                <td className='lastTd'>
                                    {
                                        isAdmin &&
                                        <>
